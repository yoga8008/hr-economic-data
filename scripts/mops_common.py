...
MOPS_OLD_ORIGIN = "https://mopsov.twse.com.tw"
...
class MopsBrowser:
    ...
    def dump_text_debug(self, name: str, text: str) -> None:
        stamp = taipei_now().strftime("%Y%m%d_%H%M%S")
        path = DEBUG_DIR / f"{stamp}_{name}.html"
        try:
            path.write_text(text or "", encoding="utf-8")
            print(f"Debug response HTML: {path}")
        except Exception as exc:
            print(f"Cannot save debug response HTML: {exc}")
    ...
    def fetch_salary_ajax_html(self, year: str, typek: str) -> str:
        """Fetch salary data from the old MOPS endpoint using the old-domain browser page.

        New MOPS only calls /mops/api/redirectToOld.  The actual HTML table is
        still generated by the old ajax_t100sb15 endpoint, but it must be called
        from the old-domain page (mopsov.twse.com.tw) with a normal browser
        session/referrer.  Calling the same ajax URL from the new SPA origin may
        return the short security page.
        """
        assert self.page is not None
        page = self.page
        old_url = f"{MOPS_OLD_ORIGIN}/mops/web/t100sb15"
        ajax_path = "/mops/web/ajax_t100sb15"

        print(f"Open old MOPS salary page before AJAX: {old_url}")
        ...
        try:
            print(f"Old MOPS current URL: {page.url}")
        except Exception:
            pass
        ...
        def looks_useful(text: str) -> bool:
            ...
            if is_security_page(text):
                return False
            return any(k in text for k in ["公司代號", "公司名稱", "員工薪資", "員工人數", "查無資料"])

        # Strategy A: same-origin fetch from mopsov page.
        fetch_script = r"""
        async ({params, ajaxPath}) => {
          const body = new URLSearchParams(params).toString();
          const resp = await fetch(ajaxPath, {
            method: 'POST',
            credentials: 'include',
            headers: {
        ...
        for variant_name, params in [
            ("old-domain-fetch", base_params),
            ("old-domain-fetch-encoded", {**base_params, "encodeURIComponent": "1"}),
        ]:
            try:
                result = page.evaluate(fetch_script, {"params": params, "ajaxPath": ajax_path})
                status = result.get("status") if isinstance(result, dict) else "?"
                url = result.get("url") if isinstance(result, dict) else "?"
                text = result.get("text") if isinstance(result, dict) else ""
                print(f"Browser AJAX salary response ({variant_name}): TYPEK={typek}, RYEAR={year}, status={status}, url={url}, length={len(text)}")
                if looks_useful(text):
                    return text or ""
                self.dump_text_debug(f"salary_ajax_{variant_name}_unexpected_{year}_{typek}", text or "")
            except Exception as exc:
                print(f"Browser AJAX fetch variant failed ({variant_name}): {exc}")

        # Strategy B: real form POST navigation.  This mimics a normal browser
        # submission and often passes checks that block fetch/XHR.
        form_script = r"""
        ({params, ajaxPath}) => {
          const old = document.getElementById('__mops_salary_post_form');
          if (old) old.remove();
          const form = document.createElement('form');
          form.id = '__mops_salary_post_form';
          form.method = 'POST';
          form.action = ajaxPath;
        ...
        """
        for variant_name, params in [
            ("old-domain-form", base_params),
            ("old-domain-form-encoded", {**base_params, "encodeURIComponent": "1"}),
        ]:
            try:
                page.goto(old_url, wait_until="domcontentloaded", timeout=60000)
                page.wait_for_timeout(1200)
                print(f"Submit old MOPS ajax_t100sb15 by real form: {variant_name}, TYPEK={typek}, RYEAR={year}")
                page.evaluate(form_script, {"params": params, "ajaxPath": ajax_path})
                ...
                try:
                    ...
                    pass
                text = page.content()
                print(f"Browser form salary response ({variant_name}): current_url={page.url}, length={len(text)}")
                if looks_useful(text):
                    return text or ""
                self.dump_text_debug(f"salary_ajax_{variant_name}_unexpected_{year}_{typek}", text or "")
            except Exception as exc:
                print(f"Browser form variant failed ({variant_name}): {exc}")
